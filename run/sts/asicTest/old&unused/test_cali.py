...
import uhal
from ROOT import gROOT, TCanvas, TF1, TTree, TFile
import sys
import pickle
import time
import struct
...
###
frame_buffer = []
#for each charge injection
nnch = [0 for d in range(ch_min,ch_max, test_delta_ch)]
nadc = [0 for d in range(ch_min,ch_max, test_delta_ch)]
ntdc = [0 for d in range(ch_min,ch_max, test_delta_ch)]
nhit = [0 for d in range(ch_min,ch_max, test_delta_ch)]
   ...
   ##Mask disable
    ...
    for ch in range (ch_min,ch_max, test_delta_ch):
     ...
     # sts.write_check(130,11,0)
      ...
      print "Resetting ADC counters"
      print "Reset channel fifos"
      sts.write_check(192,2,106)
      time.sleep(0.00001)
      sts.write_check(192,2,0)
      sts.write_check(130,11,0)
      nhit[ch_cnt]  = 0 

      ##sun reset FIFO again 
      #print "nhit=",'{:8d}'.format(nhit[ch_cnt])
      #sts_com[0].reset_raw_fifo()
      ...
      #get adc,tdc,hit for each pulse injection
      #for d in range(ch_min,ch_max, test_delta_ch)
            #nhit[d]  = 0 
      # Read the raw data IPbus FIFO
      frameNb = int( sts_com[0].read_ipb_rawdata_fifo_len() )
      data_buffer = sts_com[0].read_ipb_raw_data_fifo( frameNb )
      ...
      for frame in frame_buffer :
         ...
         if frame_type:
           ...
           if 1:
             nnch[ch_cnt] = ((frame & 0x007F0000 ) >> 16 )
             nadc[ch_cnt] = ((frame & 0x0000F800 ) >> 11)
             ntdc[ch_cnt] = fullTs
             nhit[ch_cnt] += 1 
             #print '{:8d}'.format(ch_cnt),
             #print '{:8d}'.format(nhit[ch_cnt]),
             #print '{:8d}'.format(nnch[ch_cnt]),
             #print '{:8d}'.format(nadc[ch_cnt]),
             #print '{:8d}'.format(ntdc[ch_cnt])
    ...
    filename = "./calibration/cali_" +date+"_"+str(testch)
    ...
    for ch in range (ch_min, ch_max, test_delta_ch):
        ...
        f1.write('{:8d}'.format(nadc[ch_cnt]))
        f1.write('{:8d}'.format(ntdc[ch_cnt]))
        f1.write('{:8d}'.format(nhit[ch_cnt]))
        print '{:8d}'.format(nadc[ch_cnt]),
        print '{:8d}'.format(ntdc[ch_cnt]),
        print '{:8d}'.format(nhit[ch_cnt]),
