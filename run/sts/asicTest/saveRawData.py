...
#################################################
rootfilename = "data/XYTER_%s_for%ss.root"%(runid, sys.argv[3])
ofile = TFile(rootfilename,'RECREATE')
tree = TTree('otree','fifo tree')
##maximum 32 hit in one frame
maxn = 2
nhit = array('i', [ 0 ])
nch =  array('i', maxn*[ 0 ])
nadc =  array('i', maxn*[ 0 ])
...
tree.Branch('nch',nch, 'nch[nhit]/I')
tree.Branch('nadc',nadc, 'nadc[nhit]/I')
tree.Branch('ntdc',ntdc, 'ntdc[nhit]/I')
################################################
...
######################################################
def save_frame( frame , tree, nhit):
  frame_type = frame & 0x00800000
  frame_subtype = frame & 0x00400000
  if frame_type:
    if frame_subtype:
    ...
    if 1:
      hittmp = nhit[0]
      nch[ hittmp ] = ((frame & 0x007F0000 ) >> 16 )
      nadc[ hittmp ] = ((frame & 0x0000F800 ) >> 11)
      ntdc[ hittmp ] = fullTs
      nhit[0] += 1
      tree.Fill()
      #print 'Raw 0x{:08x}'.format( frame ),
...
for frame in frame_buffer :
    n_frame += 1
    #print '>>>>>>>>>>>>>>>>>>>>>>>frame  {:3d}'.format( n_frame )
    print_frame( frame )
    nhit[0] = 0
    for hit in range( 0, maxn):
         nch[hit]  = -999
         nadc[hit] = -999
         ntdc[hit] = -999
    save_frame( frame , tree , nhit)
...
for chan in range( 0, 128):
  if 0 < ch_count[ chan ] :
    ...
    hit_total += ch_count[ chan ]

print 'hit_total   {:5d}'.format( hit_total )
log.info("----------------------------------------------------------------")


outfilename = "data/binarydump_1elink_6ch_extAmpCalPhaseLock_4ch_InputPhaseScan_%s.bin"%(runid)
